...
async function connectToWA() {
    ...
    conn.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            conn.sendMessage(ownerNumber + "@s.whatsapp.net", {
                image: { url: `https://i.ibb.co/qMGrGP2/d08df2dd883986af.jpg` },
                caption: up
            })
        }
    })
    ...
    // Messages upsert event handler (මෙතනින් පහළට එකම event handler එක එක විදිහකට දෙන්න ඕන)
    conn.ev.on('messages.upsert', async (mek) => {
        try {
            ...
            // Auto-read functionality (මේක උඩ තිබුණු duplicate event handler එකෙන් ගත්තා)
            if (config.READ_MESSAGE === 'true') {
                await conn.readMessages([mek.key]);
                console.log(`Marked message from ${mek.key.remoteJid} as read.`);
            }

            // Status react functionality (මේකත් උඩ තිබුණු duplicate event handler එකෙන් ගත්තා)
            ...
            // Anti-delete functionality (මේක unchanged තියෙනවා)
            if (!m.id.startsWith("BAE5")) {
                ...
                function handleMessageRevocation(revocationMessage) {
                    ...
                    if (originalMessage) {
                        ...
                        } else if (originalMessage.message && originalMessage.message.extendedTextMessage && originalMessage.msg.text) {
            ...
            // Decode JID function (මේක unchanged තියෙනවා)
            conn.decodeJid = jid => {
                if (!jid) return jid;
                if (/:\d+@/gi.test(jid)) {
